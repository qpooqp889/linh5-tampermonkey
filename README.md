...
v3.0.14 在既有功能之外加入一組低耦合的工具層。開啟右上角設定面板後，可以使用「匯出設定」將 `lh5_` 開頭的 localStorage 設定與 Tampermonkey 設定保存為 JSON；使用「匯入設定」可在重新安裝瀏覽器或更換裝置後恢復設定。匯入時只接受本工具產生的資料格式，且只寫入 `lh5_` 前綴的鍵值，不會覆蓋其他網站資料。
...
「🛡️ 批次安定值」會從目前的背包 DOM 只掃描 `.subtab[data-c="wpn"]` 武器分頁與 `.subtab[data-c="arm"]` 防具分頁，並以該分頁內的 `.cell[data-i]` 作為實際背包 index；`.cnt` 作為持有數量。資料會依分頁、道具名稱與強化值分組，以下拉選單顯示。輸入批次數量後，程式會先切換到該道具所屬分頁，再依序每 350ms 送出一次封包。模式選擇「安定值強化」時送出 `enhanceSafeInv(index)`；選擇「一般強化」時送出 `enhanceInv(index)`。設定面板另有第二個「🔒 鎖定名稱強化」按鈕，會鎖定下拉選取的分類、完整道具名稱與強化值，例如 `武器｜巫術魔法杖 +6`；每強化一次都重新掃描該分頁的名稱與 `data-i`；強化成功時記錄同名道具的強化值升級，但仍固定回到下拉選取的原始強化值分組；強化失敗導致數量下降時允許下一件補位後繼續尋找，完全找不到剩餘目標或結果逾時才停止。診斷訊息會以 `[LH5][LOCK-DEBUG]` 開頭，包含 `START`、`FOUND`、`EMIT`、`CHECK`、`SUCCESS`、`MISSING` 與 `TIMEOUT` 標籤，方便測試時從 Console 複製完整紀錄。設定面板的「📊 強化記錄」可查看最多 500 筆歷史；成功率以已判定的成功／失敗筆數計算，若遊戲未回傳可判定的背包變化則標示為「未知」。面板提供「匯出 CSV」，會下載 `linh5-enhance-log-YYYY-MM-DD.csv`，並加入 UTF-8 BOM 以支援 Excel 正確開啟中文。
